...
PATH_COHERENCE = '/Volumes/Nexus/Commits/Vibration_artifacts/2020-08-05-audio_p-coherence'
...
roussel_db$pp_criterion_value[roussel_db$pp_criterion_value>5] <- 5
roussel_db$pp_criterion_value_fdr[roussel_db$pp_criterion_value_fdr>5] <- 5
...
ggsave('fig/A04_01_roussel-vs-coherence-correlation.png',width=14, height=10)
...
agg <- db %>%
  ...
  mutate(rho_text = paste0(rho_text, ifelse(rho_pvalue<0.05,'*',''))) %>%
  mutate(rho_text = paste0(rho_text, ifelse(rho_pvalue<0.01,'*',''))) %>%
  mutate(rho_text = paste0(rho_text, ifelse(rho_pvalue<0.001,'*','')))                  
...
ggsave('fig/A04_01_roussel-vs-coherence-correlation-with-rho.png',width=14, height=10)

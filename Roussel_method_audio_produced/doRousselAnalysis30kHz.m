function doRousselAnalysis30kHz(SUBJECT)
SUBJECT='DBS3014'; %used for development/debugging
...
%if files are not in same location as when previous scripts were run
% PATH_DATA_OLD = 'Z:\DBS';
% sync.folder = strrep(sync.folder, PATH_DATA_OLD, PATH_DATA);
% sync.folder = strrep(sync.folder, '\', '/');

%% Loading sessions from trellis
epoch = bml_annot_extend(session,10,10);

cfg=[];
cfg.epoch=epoch;
cfg.roi=sync(sync.filetype=="trellis.ns5" ,:);
cfg.electrode = electrode(electrode.filetype=="trellis.ns5" & ...
                          electrode.electrode == "ecog_154",:);
[ecog, loaded_epoch] = bml_load_epoched(cfg);

%% loading produced audio from zoom
cfg=[];
cfg.epoch=epoch;
cfg.roi=sync(sync.filetype=="audio.wav",:);
cfg.chantype = 'audio';
cfg.relabel = 'audio_p';
cfg.timetol_consolidate = 0.0011; % if you get an error, increase if necessary based on max delta value in the error message
audio_p = bml_load_epoched(cfg);

%normalizing produced audio to trellis
audio_p = bml_conform_to(ecog,audio_p);
...
for i=1:length(ecog_f.trial)
    
    
    brain_matfile_path=[PATH_ANDATA filesep SUBJECT '_S' num2str(i) '_ecog.mat'];
	audio_matfile_path=[PATH_ANDATA filesep SUBJECT '_S' num2str(i) '_audio_p.mat'];
    ...
    data = audio_p.trial{i}';
    fs = bml_getFs(audio_p);
    ...
    saveas(gcf,[PATH_ANFIG filesep SUBJECT '_displayArtifacts.png'])
    close()
	...
	saveas(gcf,[PATH_ANFIG filesep SUBJECT '_corrOverview.png'])
    close()
	saveas(gcf,[PATH_ANFIG filesep SUBJECT '_mostCorrChannels.png'])
    close()
	...
	saveas(gcf,[PATH_ANFIG filesep SUBJECT '_timelag.png'])
    close()
	...
	saveas(gcf,[PATH_ANFIG filesep SUBJECT '_statistical_criterion.png'])
    close()

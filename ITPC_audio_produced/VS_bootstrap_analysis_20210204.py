...
def get_bootstrapped_sample(variable,area='all',nboot=1000):
...
#Note: Results in comments below were run using nboot = 10,000.
#Area 43:
area = '43'
...
#Performing FDR analysis on bootstat probabilities:
p_values = np.zeros(15)
p_values[0] = 1 - p_43[0]
p_values[1] = 1 - p_44[0]
p_values[2] = p_55b[0]
p_values[3] = 1 - p_6r[0]
...
p_values[7] = 1 - p_A4[0]
p_values[8] = p_A5[0]
p_values[9] = 1 - p_OP4[0]
p_values[10] = p_p9_46v[0]
p_values[11] = 1 - p_PF[0]
p_values[12] = p_PFop[0]
p_values[13] = p_PSL[0]
p_values[14] = p_STV[0]

p_values = np.sort(p_values)

q_values = np.zeros(15)
for i in np.arange(np.shape(p_values)[0]):
    q_values[i] = p_values[i]*15/(i+1)

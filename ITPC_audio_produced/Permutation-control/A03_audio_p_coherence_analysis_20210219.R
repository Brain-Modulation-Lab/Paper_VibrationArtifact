...
PATH_ANALYSIS = "/Volumes/Nexus/Commits/Vibration_artifacts/audio_p-coherence_syllable_triplet/Permutation-control"
...
range(coherence_agg$rNC,na.rm=TRUE)
quantile(coherence_agg$rNC,probs=0.9999,na.rm=TRUE)
...
coherence_agg %>%
  filter(rNC < 20) %>%
  with(quantile(rNC,probs=c(0.95,0.99,0.999,0.99974)))
...
ggsave('fig/03_norm_coherence_tile.png',width=15,height=20)

...
xcorr_db <- read_tsv('data/xspectrogram_by_electrode.tsv') %>%
  filter(type=='prod') %>%
  select(-electrode2)
...
db %>%
  ...
  ggplot() +
  aes(x=rNC,y=xcorr,color=electrode_type)+
  geom_point(alpha=0.5,size=0.8)+
  facet_wrap(~sub_ses) 
ggsave('fig/A09_01_xcorr-vs-coherence-correlation.png',width=14, height=10)

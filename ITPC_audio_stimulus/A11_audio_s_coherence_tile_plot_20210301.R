...
PATH_ANALYSIS = "/Volumes/Nexus/Commits/Vibration_artifacts/audio_s-coherence_syllable_triplet"
#PATH_ANALYSIS = "Z:/Commits/Vibration_artifacts/audio_s-coherence_syllable_triplet"
...
iepc_db %>%
  ...
  coord_equal()
ggsave('fig/A11_coherence_raster_3000_series_phase cluster_accros_ecog1&2_contacts.pdf',width=12,height=3)

iepc_agg <- iepc_db %>%
  group_by(electrode_type2,subject) %>%
  summarise(iepc=abs(mean(mNC/abs(mNC))),n=n()) %>%
  ungroup() %>%
  mutate(p_value = exp(-n*(iepc^2)))

%function P10b_time_frequency_arC_lock_speech_fft_20201228(SUBJECT)
...
TF_RATE = 50; %Hz Sampling rate of time frequency plot
%TF_FOI = round(10.^(0.30:0.05:2.4),2,'signif');
TF_FOI = 2:1:250;
TF_BASELINE_WIDTH = 0.5;

%defining paths
%SUBJECT='DBS3029'; %used for development/debugging
...
PATH_SYNC = [PATH_SUBJECT filesep 'Preprocessed Data/Sync/annot'];

%PATH_ANALYSIS = 'Z:\Commits\Vibration_artifacts\vibration_sensor';
PATH_ANALYSIS = '/Volumes/Nexus/Commits/Vibration_artifacts/vibration_sensor';
...
session = bml_annot_read([PATH_SYNC filesep SUBJECT '_session.txt']);
electrode = bml_annot_read([PATH_SYNC filesep SUBJECT '_electrode.txt']);
artifact = bml_annot_read([PATH_SYNC filesep SUBJECT '_artifact_criteria_' CRITERIA '.txt']);
empty_electrode = bml_annot_read([PATH_SYNC filesep SUBJECT '_empty_electrode.txt']);
produced_triplet = bml_annot_read([PATH_SYNC filesep SUBJECT '_produced_triplet.txt']);
glitches = bml_annot_read([PATH_SYNC filesep SUBJECT '_glitches.txt']);
vibr_glitches = bml_annot_read([PATH_SYNC filesep SUBJECT '_glitches_vibr.txt']);
...
load([PATH_SUBJECT filesep 'Preprocessed Data/FieldTrip' filesep SUBJECT '_ft_raw_session.mat'],'D');
   
%% removing channels with audio envelope
cfg=[];
%cfg.channel = {'all' '-envaudio_p' '-envaudio_s'}; 
cfg.channel = {'vibr' 'audio_p' 'audio_s'}; 
...
cfg.complete_trial = false; %masksing glitches
cfg.value=0;
D1 = bml_mask(cfg,D1);
...
cfg=[];
cfg.groupby='label';
glitches2 = bml_annot_union(cfg, glitches, vibr_glitches);

%crossfading glitches
cfg=[];
cfg.annot = glitches2;
cfg.label_colname = 'label';
cfg.crossfade = 200;
cfg.value = 0;
D_filt_cf = bml_mask(cfg,D_filt);

%bml_praat(D_filt,D_filt_cf)
...
cfg.value=NaN;
D_filt_mask = bml_mask(cfg,D_filt_cf);
...
cfg.t_ftimwin = 4 .* dt .* ones(1,length(TF_FOI));
cfg.keeptrials   = 'yes';
...
for s=1:height(session)
  ...
  elec = D_trial_ref.label;
  
  eltypes = {    'ecog',  'micro','macro','physio','audio'};
  eltypes_zlim = {[-5, 5], [-5,5], [-5,5], [0,4]     ,[]};
  other_zlim = {[-10,40]};
  ...
  for e = 1:length(elec)
    ...
    electrode_type = bml_map(elec(e),electrode.electrode,electrode.type,'audio');
    zlim = bml_map(electrode_type,eltypes,eltypes_zlim,other_zlim);
...
cfg.complete_trial = false; %masks entire trials
cfg.value=0;
D2 = bml_mask(cfg,D_filt);

%
cfg=[];
bml_praat(ft_selectdata(cfg,D2))

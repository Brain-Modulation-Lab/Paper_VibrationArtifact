function P10b_time_frequency_get_spect(SUBJECT)
...
archive_name=[PATH_TF_OUT filesep SUBJECT '_xspectrogram.csv'];
archive=fopen(archive_name, 'w');
fprintf(archive, '%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t\n', 'session', 'n_ch', 'global_xcorr', 'iti_xcorr', 'stim_xcorr', 'prod_xcorr');
...
%   
  ...
  % save fig
  export_fig([PATH_TF_FIG filesep SUBJECT '_S' num2str(s) '_' 'audio_p'  '_TF_depth_ar_' CRITERIA '_nobaseline.png'],'-m3',gcf);
  close(gcf);
  ...
  for e = 1:length(elec)
    ...
    %save fig
    export_fig([PATH_TF_FIG filesep SUBJECT '_S' num2str(s) '_' elec(e) '_TF_depth_ar_' CRITERIA '_zscore.png'],'-m3',gcf);
    close(gcf);
    ...
    % global
    out_xspeccorr{e}=get_xsimilarities(s_audio, s_elect, "corr2");
    
    % get corr in stim
    ti=table2array(event_annot(2,2));
    ts=table2array(event_annot(2,3));
    time_spect=TF_speech_fft.time;
    idx_i=find(time_spect>ti,1);
    idx_s=find(time_spect>ts,1);
    s_audio_stim=s_audio(:, idx_i:idx_s);
    s_elect_stim=s_elect(:, idx_i:idx_s);
    out_xcorr_stim{e}=get_xsimilarities(s_audio_stim, s_elect_stim, "corr2");
    
    % get corr in prod
    ti=table2array(event_annot(4,2));
    ts=table2array(event_annot(4,3));
    time_spect=TF_speech_fft.time;
    idx_i=find(time_spect>ti,1);
    idx_s=find(time_spect>ts,1);
    s_audio_prod=s_audio(:, idx_i:idx_s);
    s_elect_prod=s_elect(:, idx_i:idx_s);
    out_xcorr_prod{e}=get_xsimilarities(s_audio_prod, s_elect_prod, "corr2");
    
    % get corr in IT_pRE
    ti=table2array(event_annot(1,2));
    ts=table2array(event_annot(1,3));
    time_spect=TF_speech_fft.time;
    idx_i=find(time_spect>ti,1);
    idx_s=find(time_spect>ts,1);
    s_audio_iti=s_audio(:, idx_i:idx_s);
    s_elect_iti=s_elect(:, idx_i:idx_s);
    out_xcorr_iti{e}=get_xsimilarities(s_audio_iti, s_elect_iti, "corr2");

    out_session{e}=s;
    out_elec{e}=cell2mat(elec(e));
    xspectrogram=get_xsimilarities(s_audio, s_elect, "pixel-xcorr");    
    % save figure    
    TF_xspeech_se=TF_speech_se;
    TF_xspeech_se.powspctrm=reshape(xspectrogram, [1, 1, size(xspectrogram, 1),size(xspectrogram, 2)]);
    
    cfg = [];
    cfg.baseline     = tf_baseline_speech_t0; 
    
    cfg.channel      = elec(e);
    cfg.title        = [SUBJECT ' S' int2str(s) ' ' elec{e} ' xspectrogram'];
    cfg.masknans     = 'yes';
    
    cfg.baselinetype   = 'zscore';
    ft_singleplotTFR(cfg,  TF_xspeech_se);
    export_fig([PATH_TF_FIG filesep SUBJECT '_S' num2str(s) '_' elec{e}  '_TF_depth_ar_' CRITERIA '_xspectrogram_zscore.png'],'-m3',gcf);
    close(gcf);
    ...
    % save output
    if ~isempty(out_elec)
        for ll = 1:size(out_elec,2)
            fprintf(archive, '%i\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t\n',...
                out_session{ll}, out_elec{ll}, out_xspeccorr{ll}, out_xcorr_iti{ll}, out_xcorr_stim{ll}, out_xcorr_prod{ll});
        end
    end
end
fclose(archive);

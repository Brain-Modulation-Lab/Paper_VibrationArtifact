...
PATH_COHERENCE = 'Z:/Commits/Vibration_artifacts/2020-08-05-audio_p-coherence'
...
roussel_db$pp_criterion_value[roussel_db$pp_criterion_value>5] <- 5 + runif(sum(roussel_db$pp_criterion_value>5),-0.5,0.5)
roussel_db$pp_criterion_value_fdr[roussel_db$pp_criterion_value_fdr>5] <- 5 + runif(sum(roussel_db$pp_criterion_value_fdr>5),-0.5,0.5)
...
coherence_db <- read_tsv(file.path(PATH_COHERENCE,'data/coherence_audio_p_by_electrode.tsv'))


db <- roussel_db %>%
  left_join(select(coherence_db,-electrode_type),by=c('subject','session_id','electrode')) 

db %>%
  mutate(sub_ses = paste0(subject,'_S',session_id)) %>%
  ggplot() +
  aes(x=log10(rNC),y=pp_criterion_value_fdr,color=electrode_type)+
  geom_point(alpha=0.5,size=0.8)+
  facet_wrap(~sub_ses)  +
  geom_vline(xintercept=log10(3),color='red',linetype=2) +
  geom_hline(yintercept=-log10(0.05),color='red',linetype=2)
ggsave('fig/A04_01_roussel-vs-coherence-correlation.png',width=20, height=20)


agg <- db %>%
  filter(!is.na(dataset_measure)) %>%
  filter(!is.na(rNC)) %>%
  group_by(subject,session_id,electrode_type) %>%
  mutate(n_group=n()) %>%
  filter(n_group > 10) %>%
  filter(electrode_type=='ecog') %>%
  summarise(rho = cor.test(rNC,pp_criterion_value,method='spearman')$estimate,
            rho_pvalue = cor.test(rNC,pp_criterion_value_fdr,method='spearman')$p.value) %>%
  ungroup() %>%
...
db %>%
  ...
  geom_vline(xintercept=log10(3),color='red',linetype=2) +
  geom_hline(yintercept=-log10(0.05),color='red',linetype=2) +
  geom_text(aes(label=rho_text),data=agg,x=-1.5,y=5,color='black',size=3,hjust=0,vjust=1)
ggsave('fig/A04_01_roussel-vs-coherence-correlation-with-rho.pdf',width=20, height=20)
...
db %>%
  ...
  filter(electrode_type == 'ecog') %>%
  ggplot() +
  aes(x=rNC,y=pp_criterion_value_fdr)+
  geom_point()+
...
ggsave('fig/A04_04_xcorr-vs-coherence-correlation-DBS3014-ECOG.pdf',width=3, height=3)
...
db %>%
  ...
  scale_y_continuous(breaks = 0:5, limits=c(0,5)) +
  scale_x_continuous(limits=c(0,30))
ggsave('fig/A04_04_xcorr-vs-coherence-correlation-DBS3006-ECOG.pdf',width=3, height=3)


db %>%
  filter(criterion_value_fdr<1e-4) %>%
  filter(rNC<0.2)

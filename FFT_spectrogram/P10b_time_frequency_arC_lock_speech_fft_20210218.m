...
for s=1:height(session)
  ...
  for e = 1:length(elec)
    ...
    TF_speech_fft_se = ft_freqbaseline(tmpcfg, TF_speech_fft_se);

    %run spectrogram analysis
    aux=ft_freqdescriptives([], TF_speech_fft_se);
    s_elect=squeeze(aux.powspctrm);
        
    % electrode plot
    figure()
    set(gcf, 'Position',  [100, 100, 250, 200])
    imagesc(TF_speech_fft_se.time, TF_speech_fft_se.freq,s_elect);
    shading interp
    set(gca,'YDir', 'normal');
    hold on
    xline(TF_speech_fft_se.time(TF_speech_fft_se.time==0), '--r', 'LineWidth', 1.25)
    title(elec{e})
    colorbar()
    
    export_fig([PATH_TF_FIG filesep SUBJECT '_S' num2str(s) '_' elec{e}  '_TF_depth_ar_' CRITERIA '_fft_zscore.png'],'-m3','-transparent',gcf);
    close(gcf);
